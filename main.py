...
client = TelegramClient('mr_white_master_v31', config.API_ID, config.API_HASH, connection_retries=None)

# --- 1. ADMIN COMMANDS (CHECKED FIRST) ---
...
async def status_cmd(event):
    # This now runs correctly because it is placed before the forwarding handler
...
async def handle_incoming(event):
    # This handler only runs if the message didn't match any command above
