...
client = TelegramClient('mr_white_final_v27', config.API_ID, config.API_HASH, connection_retries=None)

# --- 1. ADMIN: MANAGEMENT & SEARCH ---
...
async def broadcast(event):
    ...
    if blocked_list:
        await client.send_message(config.ADMIN_ID, f"🚫 **/blocked List:**\n`{', '.join(blocked_list)}`")

# --- 3. START COMMAND (USER COUNTING & WELCOME) ---
...
# --- 4. FORWARDING (FIXED SLEEP PRIORITY) ---
...
async def callback_handler(event):
    ...
    if data == "claim_pay":
        ...
        await client.send_message(uid, "✅ **Payment Verified!** Access granted for 24h.")
