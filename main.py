...
client = TelegramClient('mr_white_master_v47', config.API_ID, config.API_HASH)
...
# --- 2. ADMIN COMMANDS (SMART REPLY & BROADCAST) ---
...
async def admin_interface(event):
    ...
    if text.startswith('/sleep'):
        if 'off' in text:
            sleep_mode_active = False
            await event.reply("☀️ **Sleep Mode Disabled. Notifying subscribers...**")
            conn = database.get_connection(); cur = conn.cursor()
...
# --- 3. USER HANDLER (START / STATUS / SUPPORT / FORWARD) ---
